...
##' feed (food unless the element is in feedCommodities).
##' 
...
finalSuaBalance = function(data, standParams, feedCommodities = c(),
    ...
    data[abs(imbalance) > 10 & (!get(standParams$itemVar) %in% primaryCommodities),
         Value := ifelse(
            ...
            (element == p$foodCode & !(get(p$itemVar) %in% c(indCommodities, feedCommodities))),
            newValue, Value)]
